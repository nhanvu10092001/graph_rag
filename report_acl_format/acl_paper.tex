...
\FOR{$l = 0$ \textbf{to} $L_{\max}-1$}
    ...
    \FORALL{community $c \in P_l$}
        ...
        \STATE $\texttt{report} \leftarrow \texttt{LLM\_Summarize}(c)$
        \STATE \COMMENT{Returns: title, summary, findings, importance\_score}
        \STATE $\mathbf{v}_c \leftarrow \texttt{embed}(\texttt{report.summary})$
        \STATE Persist $c$ to Neo4j with level $l$, embedding $\mathbf{v}_c$
    \ENDFOR
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\textbf{Cypher} & Enables precise structured queries (counts, aggregations, path queries) by translating NL to graph query language. Self-correction loop recovers from syntax errors within 3 iterations; unrecoverable queries fall back to vector retrieval. \\
