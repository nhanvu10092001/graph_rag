"""
MAG STaRK preprocessing - limited to ~6000 nodes.
...
from summary_utils import add_summary_to_mag
...
answer_nodes = set()
with open(DATA_DIR / "qa/mag/stark_qa/stark_qa.csv") as f:
...
edge_index = torch.load(DATA_DIR / "raw_graphs/mag/edge_index.pt", weights_only=True)
edge_types_tensor = torch.load(DATA_DIR / "raw_graphs/mag/edge_types.pt", weights_only=True)

with open(DATA_DIR / "raw_graphs/mag/edge_type_dict.pkl", "rb") as f:
...
os.makedirs(DATA_DIR / "graphs/mag/", exist_ok=True)
edges_df.to_parquet(DATA_DIR / "graphs/mag/edges.parquet")
del edges_df
gc.collect()
...
with open(DATA_DIR / "raw_graphs/mag/node_info.pkl", "rb") as f:
...
nodes_df[["index", "name", "type", "summary"]].to_parquet(DATA_DIR / "graphs/mag/nodes.parquet")

print(f"Done! Saved {len(nodes_df)} nodes to {DATA_DIR / 'graphs/mag/'}")
