...
**HLG LLM Utils** is a modular, high-performance Graph RAG (Retrieval-Augmented Generation) and multi-agent AI framework. It features hierarchical Leiden/Louvain community detection, entity/relationship extraction, self-correcting Cypher query generation, map-reduce global search, adaptive knowledge retrieval (ARK), doc parsing with OCR capabilities, and an interactive React 19 interface.
...
- **Docker Desktop** or **Docker Engine & Docker Compose v2+** (for containerized setup)
- **Node.js 20+** and **npm** (for local frontend development)
- **Python 3.11+** and **`uv`** package manager (for local backend development)
- An **OpenAI API Key** (`sk-proj-...`) or **Anthropic API Key** (`sk-ant-...`)

---

## 🚀 Quick Start with Docker Compose (Recommended)

Run the entire Graph RAG system (Databases, Infinity Embedding, FastAPI Backend, React Frontend) in isolated Docker containers with a single command.
...
Open `.env` in your text editor and set your LLM API keys:

```env
# Choose LLM_PROVIDER: "openai" or "claude"
LLM_PROVIDER=openai
...
This command automatically builds the backend and frontend images, starts PostgreSQL (with pgvector), Neo4j (with APOC plugin), MinIO, Infinity Embedding, executes database schema migrations (`alembic upgrade head`), and starts the web application.
...
| **Infinity Embedding**| `graph_rag_embedding` | `8081` | [http://localhost:8081/docs](http://localhost:8081/docs) (Embedding server API) |
...
- **LLM**: Model provider selection (`claude` or `openai`), model names, temperature, max tokens.
- **Embeddings**: Provider (`openai` or `local`), embedding model (`text-embedding-3-small`, `nomic-embed-text`).
- **Reranking**: Reranker engine selection (`flashrank`, `cross_encoder`, `llm`).
