...
\title{Autonomous Knowledge Graph Exploration via ARK: Distilling Small Language Models for Adaptive Retrieval}
...
% =============================================================================
\begin{abstract}
Retrieval-Augmented Generation (RAG) over heterogeneous, text-rich Knowledge Graphs (KGs) demands a dynamic equilibrium between global discovery across unconnected entities and multi-hop relational traversal. Conventional vector-based similarity methods suffer from relational blindness and shallow coverage, whereas rigid multi-hop algorithms rely on fragile seed selection and fixed hop limits. In this paper, we study the \textbf{ARK} (\textbf{A}daptive \textbf{R}etriever of \textbf{K}nowledge) framework---an autonomous multi-agent exploration system operating over a minimal two-tool action space (\textit{Global Search} via BM25 and \textit{Neighborhood Exploration} via one-hop expansion) aggregated through \textbf{Voting Rank Fusion} (VRF). To eliminate dependency on proprietary frontier models during multi-step graph exploration, we design a trajectory mining and knowledge distillation pipeline. We harvest exploratory trajectories from Gemini~3.6~Flash filtered by Rejection Sampling ($\text{Recall@20} > 0$) and distill \texttt{Qwen3-0.6B} via QLoRA (INT4 NF4). On the biomedical \textbf{STaRK-PrimeKG} benchmark (129k nodes, 8.1M edges), the distilled \texttt{Qwen3-0.6B} converges rapidly within 120 optimization steps and achieves a competitive evaluation loss of $0.3680$. Our findings demonstrate that a sub-billion parameter model can master complex stochastic graph exploration while dramatically reducing inference costs and enabling deployment on commodity edge hardware.
\end{abstract}
...
In this work, we address the challenge of \textbf{small model specialization for agentic graph exploration}. We develop an end-to-end trajectory mining and knowledge distillation pipeline that transfers graph exploration capabilities from frontier teacher models to a lightweight, open-source sub-billion language model (\texttt{Qwen3-0.6B}).
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Teacher Trajectory Mining Pipeline:} We design an exploratory teacher distillation pipeline with \textit{Rejection Sampling} ($\text{Recall@20} > 0$), filtering noisy exploration traces from Gemini~3.6~Flash into high-quality training demonstrations.
    \item \textbf{Parameter-Efficient Specialization:} We formulate an assistant-masked Supervised Fine-Tuning (SFT) objective and implement INT4 NF4 QLoRA for \texttt{Qwen3-0.6B}, enabling deployment on edge hardware with under $2\,\text{GB}$ VRAM.
    \item \textbf{Empirical Analysis:} On the challenging biomedical \textbf{STaRK-PrimeKG} benchmark, we demonstrate that the distilled \texttt{Qwen3-0.6B} model converges rapidly within 120 steps and achieves a competitive evaluation loss, with trajectory analysis confirming sound exploration behavior.
\end{itemize}
...
\node[proc, below=0.55cm of qa] (teacher) {\textbf{Gemini 3.6 Flash (Teacher)}\\\textit{5 Agents $\times$ Queries}};
...
\caption{Trajectory Mining and Distillation Flow. Gemini 3.6 Flash teacher exploration with Rejection Sampling produces high-quality training trajectories for distilling \texttt{Qwen3-0.6B}.}
\label{fig:distill_flow}
\end{figure}

To eliminate reliance on commercial APIs, we construct a teacher-driven training regime depicted in Figure~\ref{fig:distill_flow}.

\subsection{Teacher Trajectory Mining \& Rejection Sampling}
To capture robust exploration policies, we deploy Gemini~3.6~Flash with $M=5$ parallel agents across all 592 training and 199 validation queries, generating 2,840 training and 881 validation trajectories.
...
We train the student model using an \textbf{assistant-only masked cross-entropy loss}. We assign target labels $y_t = x_t$ if token $t$ is generated by the assistant, and $y_t = -100$ (ignored in gradient computation) otherwise:
\begin{equation}
\label{eq:sft_loss}
...
\subsection{Parameter-Efficient Adaptation: \texttt{Qwen3-0.6B} QLoRA Distillation}
To enable deployment on edge GPUs with $<2\,\text{GB}$ VRAM, \texttt{Qwen3-0.6B} is quantized to 4-bit NormalFloat (NF4) with double quantization via QLoRA \citep{dettmers2023qlora}. The full hyperparameter configuration is summarized in Table~\ref{tab:full_hypers}.
...
\begin{tabular}{lc}
\toprule
\textbf{Hyperparameter} & \textbf{Qwen3-0.6B Distillation} \\
\midrule
Base Model & \texttt{Qwen/Qwen3-0.6B} \\
Precision & INT4 (NF4) \\
LoRA Rank ($r$) & 32 \\
LoRA Alpha ($\alpha$) & 64 \\
LoRA Dropout & 0.1 \\
...
Optimizer & AdamW \\
Loss Formulation & Assistant-only \\
Rejection Sampling & Yes ($\text{Rec@20}>0$) \\
\bottomrule
\end{tabular}%
}
\caption{Hyperparameter configuration for \texttt{Qwen3-0.6B} distillation.}
\label{tab:full_hypers}
\end{table}

LoRA adapters ($r=32$, $\alpha=64$, dropout $0.1$) are attached to all linear layers, including attention projections (\texttt{q\_proj, k\_proj, v\_proj, o\_proj}) and feed-forward projections (\texttt{gate\_proj, up\_proj, down\_proj}). The model is trained over 5 epochs (120 steps) with effective batch size 16 (batch size 1, gradient accumulation 16) and learning rate $5\times 10^{-5}$ under AdamW.
...
\midrule
\textbf{Qwen3-0.6B QLoRA Distilled (Ours)} & Val & 1 & 20 & 0.1000 & 0.1500 & 0.1500 & 0.1200 & 0.1350 & 0.1180 \\
\bottomrule
\end{tabular}%
}
\caption{Retrieval performance on STaRK-PrimeKG across Oracle, Teacher, and Distilled student model. Multi-agent ensembles use Voting Rank Fusion ($\kappa=60$).}
\label{tab:main_results}
\end{table*}

\subsection{Main Retrieval Results}
Table~\ref{tab:main_results} reports retrieval performance across baselines and our distilled student model.
...
\paragraph{3. Distilled Student Model:}
Our distilled \texttt{Qwen3-0.6B} model, trained via QLoRA on rejection-sampled Gemini teacher trajectories, achieves a final validation evaluation loss of $\mathbf{0.3680}$ after 120 steps. On the validation set, the distilled model achieves $\text{Hit@1} = 0.1000$ and $\text{MRR} = 0.1180$. While lower than the teacher model, it demonstrates that a compact architecture can successfully internalize and execute multi-step graph exploration. The training dynamics, analyzed in Section~\ref{sec:training_dynamics}, confirm stable convergence and effective absorption of teacher exploration policies.
...
20 & 0.83 & 0.6472 & -- & $3.20 \times 10^{-5}$ \\
50 & 2.08 & 0.3226 & 0.3916 & $3.90 \times 10^{-5}$ \\
70 & 2.92 & 0.3263 & -- & $2.80 \times 10^{-5}$ \\
100 & 4.17 & 0.2741 & 0.3698 & $1.17 \times 10^{-5}$ \\
\textbf{120} & \textbf{5.00} & \textbf{0.2644} & $\mathbf{0.3680}$ & $5.56 \times 10^{-7}$ \\
\bottomrule
\end{tabular}%
}
\caption{Training and validation loss progression during distillation of \texttt{Qwen3-0.6B}.}
\label{tab:training_loss}
\end{table}

\subsection{Training Dynamics and Convergence Analysis}
\label{sec:training_dynamics}
Table~\ref{tab:training_loss} presents the training progression for the distilled \texttt{Qwen3-0.6B} model.

\paragraph{Generalization in \texttt{Qwen3-0.6B} Distillation:}
Training loss decreases steadily from $0.6472$ at step 20 to $0.2644$ across 5 epochs. Validation loss improves monotonically ($0.3916 \to 0.3698 \to \mathbf{0.3680}$), confirming that rejection sampling effectively eliminates noisy trajectories and prevents student overfitting. Despite having only 0.6B parameters, the model successfully internalizes the teacher's multi-step graph exploration policy, demonstrating that compact architectures can absorb complex agentic reasoning patterns when trained on high-quality filtered demonstrations.
...
Table~\ref{tab:compute_efficiency} compares the operational footprint of the frontier API teacher against our distilled local model.
...
\caption{Serving resource footprint and latency comparison between teacher and distilled student.}
\label{tab:compute_efficiency}
\end{table}

While the proprietary teacher API requires network round-trips and incurs recurring token fees, our distilled \texttt{Qwen3-0.6B} model occupies only $\sim 1.2\,\text{GB}$ of VRAM under INT4 quantization, executing single-agent queries in under $300\,\text{ms}$ on commodity hardware---a $16\times$ speedup over the 5-agent teacher ensemble.
...
In this paper, we investigated the ARK framework for autonomous multi-agent knowledge graph exploration and addressed the challenge of small language model specialization through knowledge distillation. By constructing a rejection-sampled teacher trajectory pipeline from Gemini~3.6~Flash, we successfully distilled the exploration policy into \texttt{Qwen3-0.6B} via INT4 NF4 QLoRA. Empirical evaluation on STaRK-PrimeKG confirms rapid convergence within 120 optimization steps and stable generalization across validation trajectories. Crucially, the distilled model occupies only $\sim 1.2\,\text{GB}$ of VRAM and answers queries in under $300\,\text{ms}$, making it practical for deployment on commodity edge hardware---a dramatic reduction in inference latency and cost relative to frontier API-based teacher systems.
